...
async function ensureGitHubUserMapping(githubUser: Record<string, any>): Promise<Record<string, any> | null> {
  ...
  try {
    ...
    console.error('Failed to map GitHub user:', (error as Error).message);
    return null;
  }
}
...
async function handlePullRequestEvent(payload: Record<string, any>): Promise<void> {
...
async function processPullRequestAction(action: string, pull_request: Record<string, any>, patch: Record<string, any>): Promise<void> {
  switch (action) {
    case 'closed':
...
async function handleExternalPullRequest(payload: Record<string, any>): Promise<void> {
  const { action, pull_request, repository } = payload;
...
async function handlePullRequestReviewEvent(payload: Record<string, any>): Promise<void> {
...
async function writeReviewToStream(repository: Record<string, any>, pull_request: Record<string, any>, review: Record<string, any>, reviewerMapping: Record<string, any> | null): Promise<void> {
  const streamId = `gh-pr-${repository.id}-${pull_request.number}`;
...
async function storeHumanReview(patchId: string, review: Record<string, any>, userMapping: Record<string, any> | null): Promise<void> {
...
async function getHumanReviewerPlaceholder(githubUser: Record<string, any>): Promise<string | null> {
...
async function updateReviewerStats(agentId: string, verdict: string): Promise<void> {
  await query(`
...
async function awardReviewerKarmaOnMerge(patchId: string): Promise<void> {
...
async function trackReviewerAccuracyOnClose(patchId: string): Promise<void> {
  const reviews = await query(`
...
async function handleInstallationEvent(payload: Record<string, any>): Promise<void> {
...
async function handleInstallationRepositoriesEvent(payload: Record<string, any>): Promise<void> {
...
async function syncGitSwarmRepository(orgId: string, githubRepo: Record<string, any>): Promise<void> {
  try {
    await query(`
    ...
    console.error(`Failed to sync repository ${githubRepo.full_name}:`, (error as Error).message);
  }
}

// Handle push events (for tracking commits on streams and patches)
async function handlePushEvent(payload: Record<string, any>): Promise<void> {
...
async function handleIssuesEvent(payload: Record<string, any>): Promise<void> {
...
async function handleIssueClosedForBounty(repoId: string, issue: Record<string, any>): Promise<void> {
...
async function handleIssueCommentEvent(payload: Record<string, any>): Promise<void> {
...
async function handleGitSwarmCommand(repoId: string, issue: Record<string, any>, comment: Record<string, any>, command: string, args: string | undefined): Promise<void> {
...
async function handleBountyCommand(repoId: string, issue: Record<string, any>, comment: Record<string, any>, command: string, args: string | undefined): Promise<void> {
  ...
  switch (command.toLowerCase()) {
    case 'claim':
      ...
      try {
        ...
        console.error('Failed to claim task:', (error as Error).message);
      }
      break;
    ...
    case 'abandon':
      ...
      try {
        ...
        console.error('Failed to abandon task:', (error as Error).message);
      }
      break;
...
async function handleWorkflowRunEvent(payload: Record<string, any>): Promise<void> {
  ...
  if (_pluginEngine) {
    try {
      ...
      console.error('Failed to resolve workflow completion:', (err as Error).message);
    }
  }
}
 ...
 */
function emitGitswarmEvent(repoId: string, eventType: string, payload: Record<string, any>): void {
  if (!_pluginEngine) return;
  _pluginEngine.processGitswarmEvent(repoId, eventType, payload)
    .catch((err: any) => console.error(`Gitswarm event ${eventType} failed:`, err.message));
}
 ...
 */
async function checkAndEmitConsensus(repoId: string, streamId: string, prNumber: number, branchName: string): Promise<void> {
  try {
    const reviews = await query(`
    ...
    console.error('checkAndEmitConsensus failed:', (err as Error).message);
  }
}
 ...
 */
function _mapWebhookToAuditAction(event: string, payload: Record<string, any>): string | null {
...
async function _resolveRepoIdFromPayload(payload: Record<string, any>): Promise<string | null> {
