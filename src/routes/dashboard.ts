...
export default async function dashboardRoutes(fastify: FastifyInstance, options: Record<string, any> = {}): Promise<void> {
  ...
  fastify.get('/dashboard/activity', {
    ...
    const activity = await activityService.getRecentActivity((request.query as any));
    return { activity };
  });
  ...
  fastify.get('/dashboard/agents', {
    ...
    const { limit, offset, search, sort } = (request.query as any);
  ...
  fastify.get('/dashboard/stats/timeseries', {
    ...
    const { period = '7d', metric = 'all' } = (request.query as any);
