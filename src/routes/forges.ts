...
export async function forgeRoutes(app: FastifyInstance, options: Record<string, any> = {}): Promise<void> {
  ...
  app.patch('/forges/:id', {
    ...
    const { description, settings, consensus_threshold } = (request.body as any);
  ...
  app.post('/forges/:id/link-github', {
    ...
    const { id } = (request.params as any);
    const { repo, installation_id } = (request.body as any);
