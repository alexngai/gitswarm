...
export async function knowledgeRoutes(app: FastifyInstance, options: Record<string, any> = {}): Promise<void> {
  ...
  app.post('/hives/:name/knowledge', {
    ...
    const { claim, evidence, confidence = 0.5, citations = [], code_example } = (request.body as any);
  ...
  app.get('/hives/:name/knowledge', {
    ...
    const status = (request.query as any).status; // filter by status
  ...
  app.get('/knowledge/search', {
    ...
    const { q, hive, status, min_confidence } = (request.query as any);
