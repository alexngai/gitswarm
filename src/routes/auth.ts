...
export const sessions: Map<string, Record<string, any>> = new Map();

function generateSessionId(): string {
  return crypto.randomBytes(32).toString('hex');
}

export default async function authRoutes(fastify: FastifyInstance, options: Record<string, any> = {}): Promise<void> {
  const { db } = options;
  ...
  fastify.decorate('requireAdmin', async function(request, reply) {
    await (fastify as any).authenticateHuman(request, reply);
...
// Session store already exported above
