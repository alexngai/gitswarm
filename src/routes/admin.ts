...
function requireAdmin(request: FastifyRequest, reply: FastifyReply, done: () => void): void {
...
export default async function adminRoutes(fastify: FastifyInstance, options: Record<string, any> = {}): Promise<void> {
  const { db } = options;
  ...
  fastify.get('/admin/reports', {
    ...
    const { status = 'pending', target_type, limit = 50, offset = 0 } = (request.query as any);
  ...
  fastify.post('/admin/reports/:id/resolve', {
    ...
    const { id } = (request.params as any);
    const { action, note } = (request.body as any);
    ...
    await query(
      ...
      [status, (request as any).user.id, id]
    );
  ...
  fastify.get('/admin/agents', {
    ...
    const { status, search, sort = 'created_at', limit = 50, offset = 0 } = (request.query as any);
  ...
  fastify.post('/admin/agents/:id/ban', {
    ...
    console.log(`Agent ${id} banned by admin ${(request as any).user.id}. Reason: ${reason || 'No reason provided'}`);
  ...
  fastify.post('/admin/agents/:id/unban', async (request, reply) => {
    ...
    console.log(`Agent ${id} unbanned by admin ${(request as any).user.id}`);
