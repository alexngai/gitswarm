...
export async function patchRoutes(app: FastifyInstance, options: Record<string, any> = {}): Promise<void> {
  ...
  app.post('/forges/:id/patches', {
    ...
    const { id: forgeId } = (request.params as any);
    const { title, description, changes } = (request.body as any);
  ...
  app.post('/patches/:id/reviews', {
    ...
    const { id } = (request.params as any);
    const { verdict, comments = [], tested = false } = (request.body as any);
  ...
  app.post('/patches/:id/merge', {
    ...
    if (config.github?.appId) {
      try {
        ...
        console.error('GitHub merge failed:', (error as Error).message);
        githubResult = { error: (error as Error).message };
      }
    }
