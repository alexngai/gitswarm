...
export async function gitswarmRoutes(app: FastifyInstance, options: Record<string, any> = {}): Promise<void> {
  ...
  await app.register(swarmRoutes, { activityService });
  ...
  app.patch('/gitswarm/orgs/:id', {
    ...
    const { id } = (request.params as any);
    const { default_agent_access, default_min_karma } = (request.body as any);
  ...
  app.get('/gitswarm/repos', {
    ...
    const limit = Math.min(parseInt((request.query as any).limit) || 50, 100);
    const offset = parseInt((request.query as any).offset) || 0;
    const orgId = (request.query as any).org_id;
    const agentAccess = (request.query as any).agent_access;
    const language = (request.query as any).language;
    const search = (request.query as any).q;
    const sort = (request.query as any).sort || 'updated_at';
    const order = (request.query as any).order === 'asc' ? 'ASC' : 'DESC';
  ...
  app.patch('/gitswarm/repos/:id', {
    ...
    if (hasConfigFile) {
      const conflicting = REPO_OWNED_FIELDS.filter(f => (request.body as any)[f] !== undefined);
  ...
  app.post('/gitswarm/repos/:id/maintainers', {
    ...
    const { id } = (request.params as any);
    const { agent_id, role = 'maintainer' } = (request.body as any);
  ...
  app.delete('/gitswarm/repos/:id/maintainers/:agent_id', {
    ...
    const { id, agent_id } = (request.params as any);
  ...
  app.post('/gitswarm/repos/:id/access', {
    ...
    const { id } = (request.params as any);
    const { agent_id, access_level, reason, expires_at } = (request.body as any);
  ...
  app.get('/gitswarm/repos/:id/contents/*', {
    ...
    const ref = (request.query as any).ref;
  ...
  app.get('/gitswarm/repos/:id/commits', {
    ...
    const { id } = (request.params as any);
    const { sha, path, since, until, per_page } = (request.query as any);
  ...
  app.get('/gitswarm/repos/:id/pulls', {
    ...
    const { id } = (request.params as any);
    const { state, sort, direction, per_page } = (request.query as any);
    ...
    try {
      const backend = await getBackendForRepo(id);
      const pulls = (backend as any).getPullRequests
  ...
  app.patch('/gitswarm/repos/:id/branch-rules/:ruleId', {
    ...
    const { id, ruleId } = (request.params as any);
  ...
  app.post('/gitswarm/repos/:id/contents/*', {
    ...
    const { id } = (request.params as any);
    const path = (request.params as any)['*'] || '';
    const { content, message, branch } = (request.body as any);
  ...
  app.put('/gitswarm/repos/:id/contents/*', {
    ...
    const { id } = (request.params as any);
    const path = (request.params as any)['*'] || '';
    const { content, message, sha, branch } = (request.body as any);
  ...
  app.post('/gitswarm/repos/:id/branches', {
    ...
    const { id } = (request.params as any);
    const { name, sha } = (request.body as any);
  ...
  app.post('/gitswarm/repos/:id/pulls', {
    ...
    const { id } = (request.params as any);
    const prData = (request.body as any);
  ...
  app.get('/gitswarm/repos/:id/pulls/:prNumber/merge-check', {
    ...
    const { id, prNumber } = (request.params as any);
  ...
  app.put('/gitswarm/repos/:id/pulls/:prNumber/merge', {
    ...
    const { id, prNumber } = (request.params as any);
    const { merge_method = 'squash', commit_title, commit_message } = (request.body as any) || {};
  ...
  app.post('/gitswarm/repos/:id/pulls/:prNumber/reviews', {
    ...
    const { id, prNumber } = (request.params as any);
    const { verdict, body, tested = false } = (request.body as any);
  ...
  app.put('/gitswarm/repos/:id/stage', {
    ...
    const { id } = (request.params as any);
    const { stage, reason } = (request.body as any);
