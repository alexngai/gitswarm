 ...
 */
export async function installRoutes(app: FastifyInstance, options: Record<string, any> = {}): Promise<void> {
  ...
  app.get('/gitswarm/install', async (request, reply) => {
    const { state, suggested_target_id } = (request.query as any);
  ...
  app.get('/gitswarm/callback', async (request, reply) => {
    ...
    try {
      ...
      const installationDetails = await (githubApp as any).getInstallationDetails(installation_id);
      ...
      const repos = await (githubApp as any).getInstallationRepositories(installation_id);
      ...
      if ((request.query as any).redirect === 'false') {
        return {
          success: true,
          org: {
            id: org.id,
  ...
  app.get('/gitswarm/install/status/:orgName', async (request, reply) => {
    const { orgName } = (request.params as any);
  ...
  app.post('/gitswarm/install/:orgId/sync', async (request, reply) => {
    ...
    try {
      ...
      const repos = await (githubApp as any).getInstallationRepositories(org.github_installation_id);
      ...
      for (const repo of repos || []) {
        try {
          ...
          console.error(`Failed to sync repo ${repo.full_name}:`, (error as Error).message);
          failed++;
        }
      }
...
async function syncRepository(orgId: string, githubRepo: Record<string, any>): Promise<void> {
  await query(`
