...
export async function pluginRoutes(app: FastifyInstance, options: Record<string, any> = {}): Promise<void> {
  ...
  app.patch('/gitswarm/repos/:id/plugins/:name', {
    ...
    const { id, name } = (request.params as any);
    ...
    for (const field of allowedFields) {
      if ((request.body as any)[field] !== undefined) {
        const value = jsonFields.includes(field)
          ? JSON.stringify((request.body as any)[field])
          : (request.body as any)[field];
  ...
  app.post('/gitswarm/repos/:id/plugins/sync', {
    ...
    try {
      ...
      return reply.status(500).send({ error: 'Sync Failed', message: (err as Error).message });
    }
  });
  ...
  app.get('/gitswarm/repos/:id/plugins/executions', {
    ...
    const pluginName = (request.query as any).plugin;
  ...
  app.post('/gitswarm/repos/:id/plugins/executions/:execId/report', {
    ...
    const { id, execId } = (request.params as any);
    ...
    const result = await pluginEngine.reportExecutionResult(execId, (request.body as any));
    return result;
  });
}
