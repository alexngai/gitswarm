...
export async function bountyRoutes(app: FastifyInstance, options: Record<string, any> = {}): Promise<void> {
  ...
  app.post('/hives/:name/bounties', {
    ...
    const { title, description, reward_karma, code_context, deadline } = (request.body as any);
  ...
  app.post('/bounties/:id/solutions', {
    ...
    const { id } = (request.params as any);
    const { solution, code } = (request.body as any);
  ...
  app.post('/bounties/:id/accept', {
    ...
    const { id } = (request.params as any);
    const { solution_id } = (request.body as any);
