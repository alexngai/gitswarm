...
const API_KEY: string | undefined = process.env.GITSWARM_API_KEY;
const REPO_ID: string | undefined = process.env.GITSWARM_REPO_ID;

/**
 * Make an authenticated request to the gitswarm API.
 */
async function apiRequest(method: string, path: string, body: Record<string, any> | null = null): Promise<any> {
...
const HANDLERS: Record<string, (args: Record<string, any>) => Promise<any>> = {
  async get_repo_config(args: Record<string, any>): Promise<any> {
  ...
  async get_consensus_status(args: Record<string, any>): Promise<any> {
    const repoId = args.repo_id || REPO_ID;
  ...
  async get_agent_karma(args: Record<string, any>): Promise<any> {
    try {
      if (args.agent_id) {
  ...
  async list_active_streams(args: Record<string, any>): Promise<any> {
    const repoId = args.repo_id || REPO_ID;
    if (!repoId) return { error: 'No repo_id' };
  ...
  async get_stream_status(args: Record<string, any>): Promise<any> {
    const repoId = args.repo_id || REPO_ID;
    if (!repoId) return { error: 'No repo_id' };
  ...
  async search_issues(args: Record<string, any>): Promise<any> {
    const repoId = args.repo_id || REPO_ID;
    if (!repoId) return { error: 'No repo_id' };
  ...
  async search_streams(args: Record<string, any>): Promise<any> {
    const repoId = args.repo_id || REPO_ID;
    if (!repoId) return { error: 'No repo_id' };
  ...
  async get_repo_activity(args: Record<string, any>): Promise<any> {
    const repoId = args.repo_id || REPO_ID;
    if (!repoId) return { error: 'No repo_id' };
  ...
  async get_stage_info(args: Record<string, any>): Promise<any> {
    const repoId = args.repo_id || REPO_ID;
    if (!repoId) return { error: 'No repo_id' };
  ...
  async report_execution(args: Record<string, any>): Promise<any> {
    const repoId = args.repo_id || REPO_ID;
    if (!repoId) return { error: 'No repo_id' };
