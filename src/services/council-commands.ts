...
export class CouncilCommandsService {
   ...
   */
  async createCouncil(repoId: string, options: CouncilOptions = {}): Promise<Record<string, any>> {
    const {
      min_karma = 1000,
      min_contributions = 5,
      min_members = 3,
   ...
   */
  async getCouncil(repoId: string): Promise<Record<string, any> | null> {
    const result = await this.query(`
      SELECT c.*,
   ...
   * Get council members
   */
  async getCouncilMembers(councilId: string): Promise<Record<string, any>[]> {
    const result = await this.query(`
   ...
   */
  async checkEligibility(agentId: string, repoId: string): Promise<{ eligible: boolean; reason?: string; required?: number; current?: number }> {
   ...
   */
  async addMember(councilId: string, agentId: string, role: string = 'member'): Promise<Record<string, any>> {
    const result = await this.query(`
   ...
   */
  async removeMember(councilId: string, agentId: string): Promise<Record<string, any>> {
    const result = await this.query(`
   ...
   */
  async updateCouncilStatus(councilId: string): Promise<void> {
   ...
   * Create a proposal
   */
  async createProposal(councilId: string, proposedBy: string, data: ProposalData): Promise<Record<string, any>> {
    const {
      title,
      description,
      proposal_type,
   ...
   * Vote on a proposal
   */
  async vote(proposalId: string, agentId: string, vote: string, comment: string | null = null): Promise<{ success: boolean; vote: string }> {
   ...
   */
  async checkProposalResolution(proposalId: string): Promise<string | null> {
    const proposal = await this.query(`
      SELECT p.*, c.id as council_id,
   ...
   */
  async resolveProposal(proposalId: string, status: string): Promise<Record<string, any>> {
    const proposal = await this.query(`
      UPDATE gitswarm_council_proposals SET
        status = $1,
        resolved_at = NOW()
   ...
   */
  async executeProposalAction(proposal: Record<string, any>): Promise<Record<string, any> | null> {
  ...
  // Action executors
  async executeAddMaintainer(repoId: string, data: Record<string, any>): Promise<Record<string, any>> {
  ...
  async executeRemoveMaintainer(repoId: string, data: Record<string, any>): Promise<Record<string, any>> {
    const { agent_id } = data;
    await this.query(`
  ...
  async executeModifyBranchRule(repoId: string, data: Record<string, any>): Promise<Record<string, any>> {
    const { rule_id, updates } = data;
    // Implementation would update branch rule
    return { executed: true, action: 'modify_branch_rule', rule_id };
  }

  async executeModifyAccess(repoId: string, data: Record<string, any>): Promise<Record<string, any>> {
  ...
  async executeChangeSettings(repoId: string, data: Record<string, any>): Promise<Record<string, any>> {
    const { settings } = data;
    // Implementation would update repo settings
    return { executed: true, action: 'change_settings', settings };
  }

  async executeMergeStream(repoId: string, data: Record<string, any>): Promise<Record<string, any>> {
  ...
  async executeRevertStream(repoId: string, data: Record<string, any>): Promise<Record<string, any>> {
    const { stream_id } = data;
  ...
  async executePromote(repoId: string, data: Record<string, any>): Promise<Record<string, any>> {
    const repo = await this.query(`
   ...
   */
  async executeCommand(parsedCommand: ParsedCommand, context: CommandContext): Promise<Record<string, any>> {
  ...
  async commandStatus(repoId: string): Promise<Record<string, any>> {
    const council = await this.getCouncil(repoId);
    if (!council) {
  ...
  async commandMembers(repoId: string): Promise<Record<string, any>> {
    const council = await this.getCouncil(repoId);
    if (!council) {
      return { message: 'No council exists for this repository' };
    }
  ...
  async commandNominate(repoId: string, nominatorId: string, nomineeRef: string): Promise<Record<string, any>> {
    const council = await this.getCouncil(repoId);
    if (!council) {
  ...
  async commandVote(repoId: string, voterId: string, proposalRef: string, voteValue: string): Promise<Record<string, any>> {
    const council = await this.getCouncil(repoId);
    if (!council) {
      return { error: 'No council exists for this repository' };
    }
  ...
  async commandPropose(repoId: string, proposerId: string, args: string[]): Promise<Record<string, any>> {
    const council = await this.getCouncil(repoId);
    if (!council) {
      return { error: 'No council exists for this repository' };
    }
   ...
   */
  async startElection(councilId: string, creatorId: string, options: ElectionOptions = {}): Promise<Record<string, any>> {
    const {
   ...
   */
  async getElection(electionId: string): Promise<Record<string, any> | null> {
    const result = await this.query(`
      SELECT e.*, c.repo_id
   ...
   */
  async getActiveElection(councilId: string): Promise<Record<string, any> | null> {
    const result = await this.query(`
   ...
   */
  async nominateCandidate(electionId: string, agentId: string, nominatedBy: string, statement: string | null = null): Promise<Record<string, any>> {
   ...
   * Accept nomination
   */
  async acceptNomination(candidateId: string, agentId: string): Promise<Record<string, any>> {
    const result = await this.query(`
   ...
   * Withdraw candidacy
   */
  async withdrawCandidacy(candidateId: string, agentId: string): Promise<Record<string, any>> {
    const result = await this.query(`
      UPDATE gitswarm_election_candidates
   ...
   */
  async getElectionCandidates(electionId: string): Promise<Record<string, any>[]> {
    const result = await this.query(`
   ...
   */
  async startVoting(electionId: string): Promise<{ success: boolean; candidates_count: number }> {
    const election = await this.getElection(electionId);
    if (!election) {
      throw new Error('Election not found');
    }
   ...
   */
  async castElectionVote(electionId: string, voterId: string, candidateId: string): Promise<{ success: boolean; voted_for: string }> {
    const election = await this.getElection(electionId);
    if (!election) {
      throw new Error('Election not found');
    }
   ...
   */
  async completeElection(electionId: string): Promise<Record<string, any>> {
    const election = await this.getElection(electionId);
    if (!election) {
      throw new Error('Election not found');
    }
   ...
   */
  async getElectionResults(electionId: string): Promise<Record<string, any>> {
    const election = await this.getElection(electionId);
    if (!election) {
      throw new Error('Election not found');
    }
   ...
   */
  async checkExpiredTerms(councilId: string): Promise<{ expired_count: number; removed: string[] }> {
