 ...
 */
function webhookToTrigger(githubEvent: string, action?: string): string {
 ...
 */
function gitswarmToTrigger(eventType: string): string {
...
export class PluginEngine {
  ...
  private safeOutputs: SafeOutputsEnforcer;

  constructor(db: DbClient, activityService: ActivityService | null) {
    this.db = db;
   ...
   */
  async processWebhookEvent(githubEvent: string, payload: Record<string, any>): Promise<Record<string, any>> {
   ...
   */
  async processGitswarmEvent(repoId: string, eventType: string, payload: Record<string, any>): Promise<Record<string, any>> {
    const enabled = await this._pluginsEnabled(repoId);
    if (!enabled) return { processed: false, reason: 'plugins_disabled' };
   ...
   */
  async _processEvent(repoId: string, trigger: string, payload: Record<string, any>): Promise<Record<string, any>> {
   ...
   */
  async _executePlugin(plugin: Record<string, any>, trigger: string, payload: Record<string, any>): Promise<Record<string, any>> {
   ...
   */
  async _executeBuiltin(plugin: Record<string, any>, trigger: string, payload: Record<string, any>, context: Record<string, any>): Promise<Record<string, any>> {
   ...
   */
  async _dispatchToGitHubActions(plugin: Record<string, any>, trigger: string, payload: Record<string, any>, context: Record<string, any>): Promise<Record<string, any>> {
    const repoId = plugin.repo_id;
   ...
   */
  async _dispatchToWebhook(plugin: Record<string, any>, trigger: string, payload: Record<string, any>, context: Record<string, any>): Promise<Record<string, any>> {
   ...
   */
  async _handleWorkflow(plugin: Record<string, any>, trigger: string, payload: Record<string, any>, context: Record<string, any>): Promise<Record<string, any>> {
   ...
   */
  async _matchPlugins(repoId: string, trigger: string): Promise<Record<string, any>[]> {
   ...
   */
  async _evaluateConditions(plugin: Record<string, any>, conditions: Record<string, any>, payload: Record<string, any>): Promise<any> {
   ...
   */
  async _evaluateConsensus(repoId: string, payload: Record<string, any>): Promise<boolean> {
   ...
   */
  async _evaluateKarma(repoId: string, payload: Record<string, any>, expected: number | string): Promise<boolean> {
    const threshold = typeof expected === 'string'
      ? parseFloat(expected.replace(/[>= ]/g, ''))
      : expected;
   ...
   */
  async _resolveRepoId(payload: Record<string, any>): Promise<string | null> {
   ...
   */
  async _pluginsEnabled(repoId: string): Promise<boolean> {
    const result = await this.db.query(`
   ...
   */
  async _addLabels(repoId: string, payload: Record<string, any>, labels: string[]): Promise<void> {
   ...
   */
  async _addComment(repoId: string, payload: Record<string, any>, body: string): Promise<void> {
    const client = await this._getGitHubRepoClient(repoId);
    if (!client) return;
   ...
   */
  async _closeCompletedTasks(repoId: string): Promise<any> {
    const result = await this.db.query(`
   ...
   */
  async _getGitHubRepoClient(repoId: string): Promise<any> {
    const repo = await this.db.query(`
      SELECT r.github_full_name, o.github_installation_id
      FROM gitswarm_repos r
      JOIN gitswarm_orgs o ON r.org_id = o.id
   ...
   */
  async _getRepoConfig(repoId: string): Promise<Record<string, any> | null> {
    const result = await this.db.query(`
   ...
   */
  async _promoteBufferToMain(repoId: string): Promise<Record<string, any>> {
    try {
   ...
   */
  async _notifyContributors(repoId: string, payload: Record<string, any>, message: string): Promise<any> {
    const issueNumber = payload.issue?.number || payload.pull_request?.number;
   ...
   */
  async _notifyStreamOwner(repoId: string, payload: Record<string, any>, message: string): Promise<any> {
   ...
   */
  async _recordExecution(plugin: Record<string, any>, trigger: string, payload: Record<string, any>, status: string, actionsTaken: string[], safeOutputSummary: Record<string, any>, errorMessage: string | null = null): Promise<void> {
    try {
      await this.db.query(`
        INSERT INTO gitswarm_plugin_executions (
   ...
   */
  async auditWorkflowAction(repoId: string, actionType: string, payload: Record<string, any>): Promise<Record<string, any>> {
    try {
   ...
   */
  async verifyExecutionToken(executionId: string, token: string): Promise<boolean> {
   ...
   */
  async resolveWorkflowCompletion(repoId: string, workflowName: string, conclusion: string): Promise<Record<string, any>> {
   ...
   */
  async reportExecutionResult(executionId: string, result: Record<string, any>): Promise<Record<string, any>> {
