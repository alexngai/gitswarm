...
const STREAM_EVENTS: Set<string> = new Set([
  'stream_created',
  'workspace_created',
  'commit',
  'submit_for_review',
...
class ActivityService {
  ...
  constructor(db: DbClient | null, wsService: WebSocketService | null) {
    this.db = db;
   ...
   * Get recent activity
   */
  async getRecentActivity(options: ActivityOptions = {}): Promise<Record<string, unknown>[]> {
    ...
    return result.rows.map((row: Record<string, unknown>) => ({
      ...
      category: STREAM_EVENTS.has(row.event_type as string) ? 'stream' : 'platform',
      agent: row.agent_id,
      agent_name: row.agent_name,
      target_type: row.target_type,
      target_id: row.target_id,
      repo_id: (row.metadata as Record<string, unknown>)?.repo_id,
      title: (row.metadata as Record<string, unknown>)?.title,
      hive: (row.metadata as Record<string, unknown>)?.hive,
      forge: (row.metadata as Record<string, unknown>)?.forge,
   ...
   */
  async getStreamActivity(repoId: string, options: StreamActivityOptions = {}): Promise<Record<string, unknown>[]> {
