...
const EMBEDDING_DIMENSIONS: number = 1536;
...
export class EmbeddingsService {
   ...
   */
  getKnowledgeText(node: any): string {
    let text = node.claim;
   ...
   */
  async searchKnowledge(query: string, options: SearchOptions = {}): Promise<Record<string, any>[]> {
   ...
   */
  async textSearchKnowledge(query: string, options: SearchOptions = {}): Promise<Record<string, any>[]> {
    const { limit = 20, hive_id, status, min_confidence } = options;
   ...
   */
  async updateNodeEmbedding(nodeId: string): Promise<boolean> {
    if (!this.enabled || !this.db) {
      return false;
    }
   ...
   */
  async backfillEmbeddings(batchSize: number = 10): Promise<{ processed: number; failed: number; total?: number }> {
    if (!this.enabled || !this.db) {
