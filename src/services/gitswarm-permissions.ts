...
interface RepoCreationLimitResult {
  allowed: boolean;
  reason?: string;
  karma?: number;
...
export class GitSwarmPermissionService extends SharedPermissionService {
   ...
   */
  async resolvePermissions(agentId: string, repoId: string): Promise<any> {
    // 1. Check explicit access
    const explicit = await (this as any).query(`
    ...
    // 2. Maintainer status
    const maintainer = await (this as any).query(`
    ...
    const repo = await (this as any).query(`
      SELECT
    ...
    // 4. Agent karma
    const agent = await (this as any).query(`SELECT karma FROM ${t('agents')} WHERE id = $1`, [agentId]);
   ...
   */
  async checkRepoCreationLimit(agentId: string): Promise<RepoCreationLimitResult> {
    const agent = await (this as any).query(`SELECT karma FROM ${t('agents')} WHERE id = $1`, [agentId]);
    ...
    const usage = await (this as any).query(`
      SELECT
