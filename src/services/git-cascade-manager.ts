...
let MultiAgentRepoTracker: any = null;
...
const DEFAULT_REPOS_DIR: string = process.env.GITSWARM_REPOS_DIR || '/var/lib/gitswarm/repos';

export class GitCascadeManager {
  private reposDir: string;
  private trackers: Map<string, TrackerContext>;
  public available: boolean;
  private pgQuery: (text: string, params?: unknown[]) => Promise<{ rows: Record<string, any>[] }>;

  constructor(reposDir: string = DEFAULT_REPOS_DIR, pgQuery: ((text: string, params?: unknown[]) => Promise<{ rows: Record<string, any>[] }>) | null = null) {
   ...
   */
  async initRepo(repoId: string, { cloneUrl, bufferBranch = 'buffer' }: { cloneUrl?: string; bufferBranch?: string } = {}): Promise<{ repoPath: string; available: boolean }> {
    if (!this.available) {
   ...
   */
  async getTracker(repoId: string): Promise<TrackerContext | null | undefined> {
   ...
   */
  async createStream(repoId: string, { agentId, name, baseBranch, parentStreamId }: { agentId: string; name?: string; baseBranch?: string; parentStreamId?: string }): Promise<{ streamId: string; branch: string; worktreePath: string }> {
   ...
   */
  async writeFile(repoId: string, agentId: string, filePath: string, content: string): Promise<{ path: string; size: number }> {
    const ctx = await this.getTracker(repoId);
    if (!ctx) throw new Error(`Repo ${repoId} not initialized for Mode C`);
   ...
   */
  async readFile(repoId: string, agentId: string, filePath: string): Promise<string | null> {
    const ctx = await this.getTracker(repoId);
    if (!ctx) throw new Error(`Repo ${repoId} not initialized for Mode C`);
   ...
   */
  async deleteFile(repoId: string, agentId: string, filePath: string): Promise<{ deleted: boolean; path?: string }> {
    const ctx = await this.getTracker(repoId);
    if (!ctx) throw new Error(`Repo ${repoId} not initialized for Mode C`);
   ...
   */
  async listFiles(repoId: string, agentId: string, dirPath: string = '.'): Promise<Array<{ name: string; path: string; type: string; size?: number }>> {
    const ctx = await this.getTracker(repoId);
    if (!ctx) throw new Error(`Repo ${repoId} not initialized for Mode C`);
   ...
   */
  async commitChanges(repoId: string, agentId: string, { message, streamId }: { message: string; streamId?: string }): Promise<{ commit: unknown; changeId: unknown; streamId: string }> {
    const ctx = await this.getTracker(repoId);
    if (!ctx) throw new Error(`Repo ${repoId} not initialized for Mode C`);
   ...
   */
  async mergeToBuffer(repoId: string, streamId: string): Promise<{ mergeCommit: string; bufferBranch: string }> {
    const ctx = await this.getTracker(repoId);
    if (!ctx) throw new Error(`Repo ${repoId} not initialized for Mode C`);
    ...
    try {
      ...
      const error: any = new Error('merge_conflict');
   ...
   */
  _detectConflicts(repoPath: string, bufferBranch: string, streamBranch: string): ConflictInfo[] {
    try {
   ...
   */
  async resolveConflict(repoId: string, streamId: string, resolutions: Resolution[]): Promise<{ mergeCommit: string; bufferBranch: string }> {
    const ctx = await this.getTracker(repoId);
    if (!ctx) throw new Error(`Repo ${repoId} not initialized for Mode C`);
   ...
   */
  async getBufferState(repoId: string): Promise<{ bufferBranch: string; bufferCommit: string; repoPath: string }> {
    const ctx = await this.getTracker(repoId);
    if (!ctx) throw new Error(`Repo ${repoId} not initialized for Mode C`);
   ...
   */
  async promote(repoId: string): Promise<{ fromCommit: string; toCommit: string; from: string; to: string }> {
    const ctx = await this.getTracker(repoId);
    if (!ctx) throw new Error(`Repo ${repoId} not initialized for Mode C`);
   ...
   */
  async pushToRemote(repoId: string, branch: string): Promise<void> {
    const ctx = await this.getTracker(repoId);
    if (!ctx) return;
