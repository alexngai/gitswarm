...
export class GitSwarmService {
  constructor(db = null) {
    ...
    this.tokenCache = new Map(); // orgId -> { token, expiresAt }
  }
   ...
   */
  async getInstallationToken(orgId) {
    // Check cache first
    ...
    const tokenData = await githubApp.getInstallationToken(installationId);
   ...
   */
  async getRepoWithCloneAccess(repoId) {
    const repo = await this.query(`
      SELECT
        r.*,
        o.github_org_name,
   ...
   */
  async getFileContents(repoId, path, ref = null) {
    const repo = await this.query(`
   ...
   */
  async getDirectoryContents(repoId, path = '', ref = null) {
    const repo = await this.query(`
      SELECT github_full_name, org_id, default_branch
      FROM gitswarm_repos WHERE id = $1
    `, [repoId]);
   ...
   */
  async getTree(repoId, ref = null, recursive = true) {
    const repo = await this.query(`
      SELECT github_full_name, org_id, default_branch
      FROM gitswarm_repos WHERE id = $1
    `, [repoId]);
   ...
   * @returns {Promise<Array<object>>}
   */
  async getPullRequests(repoId, options = {}) {
    const repo = await this.query(`
      SELECT github_full_name, org_id
      FROM gitswarm_repos WHERE id = $1
    `, [repoId]);
   ...
   */
  async createFile(repoId, path, content, message, branch, authorName, authorEmail) {
    const repo = await this.query(`
      SELECT github_full_name, org_id, default_branch
      FROM gitswarm_repos WHERE id = $1
    `, [repoId]);
   ...
   * @returns {Promise<{commit: object, content: object}>}
   */
  async updateFile(repoId, path, content, message, sha, branch, authorName, authorEmail) {
    const repo = await this.query(`
      SELECT github_full_name, org_id, default_branch
      FROM gitswarm_repos WHERE id = $1
    `, [repoId]);
   ...
   * @returns {Promise<object>}
   */
  async createBranch(repoId, branchName, fromSha) {
    const repo = await this.query(`
      SELECT github_full_name, org_id
      FROM gitswarm_repos WHERE id = $1
    `, [repoId]);
