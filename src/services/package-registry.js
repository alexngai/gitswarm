...
export class PackageRegistryService {
   ...
   */
  async getOrCreatePackage(repoId, name, packageType, metadata = {}) {
   ...
   */
  async getPackage(packageType, name) {
    const result = await this.query(`
      SELECT p.*,
   ...
   * Deprecate a package
   */
  async deprecatePackage(packageId, message) {
    const result = await this.query(`
   ...
   */
  async publishVersion(packageId, versionData, publisherId) {
    const {
      version,
      artifact,
      manifest = {},
   ...
   */
  async getVersion(packageId, version) {
    const result = await this.query(`
      SELECT v.*,
   ...
   */
  async listVersions(packageId, includeYanked = false) {
   ...
   */
  async yankVersion(packageId, version, yankerId, reason) {
    const result = await this.query(`
   ...
   * Unyank a version
   */
  async unYankVersion(packageId, version) {
    const result = await this.query(`
      UPDATE gitswarm_package_versions SET
        yanked = false,
        yanked_at = NULL,
   ...
   * Record a download
   */
  async recordDownload(versionId, metadata = {}) {
   ...
   */
  async getDownloadStats(packageId, days = 30) {
    const result = await this.query(`
   ...
   */
  async addMaintainer(packageId, agentId, role = 'maintainer', addedBy = null, permissions = {}) {
    const {
      can_publish = true,
      can_yank = true,
   ...
   */
  async removeMaintainer(packageId, agentId) {
    const result = await this.query(`
   ...
   */
  async listMaintainers(packageId) {
    const result = await this.query(`
   ...
   */
  async canPerform(agentId, packageId, action) {
   ...
   */
  async createAdvisory(packageId, data, reporterId) {
    const {
      title,
      description,
      severity,
   ...
   */
  async listAdvisories(packageId) {
    const result = await this.query(`
   ...
   */
  async storeArtifact(packageId, version, artifact) {
