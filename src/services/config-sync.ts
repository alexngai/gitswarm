...
export class ConfigSyncService {
   ...
   */
  async syncRepoConfig(repoId: string): Promise<Record<string, any>> {
   ...
   */
  async _syncRepoSettings(repoId: string, config: Record<string, any>): Promise<void> {
   ...
   */
  async _syncPlugins(repoId: string, plugins: Record<string, any>[]): Promise<any> {
   ...
   */
  async _detectWorkflowTemplates(ghRepo: GitHubRepo, branch: string, repoId: string): Promise<any> {
    const workflows = [];
  ...
  _extractAllWorkflowTriggers(on) {
    ...
    for (const [key, config] of Object.entries(on) as [string, any][]) {
  ...
  async _recordSyncError(repoId: string, error: string): Promise<void> {
    await this.db.query(`
