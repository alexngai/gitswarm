...
export class GitHubRepo {
  ...
  async request(method: string, endpoint: string, body: Record<string, any> | null = null): Promise<Record<string, any> | null> {
  ...
  // Get repository info
  async getRepo(): Promise<Record<string, any> | null> {
  ...
  async getRef(ref: string): Promise<Record<string, any> | null> {
    return this.request('GET', `/repos/${this.owner}/${this.repo}/git/refs/heads/${ref}`);
  }

  // Create a new branch from a base branch
  async createBranch(branchName: string, baseBranch: string | null = null): Promise<Record<string, any> | null> {
  ...
  // Get file content
  async getFileContent(path: string, branch: string | null = null): Promise<{ content: string; sha: string } | null> {
  ...
  async createOrUpdateFile(path: string, content: string, message: string, branch: string, existingSha: string | null = null): Promise<Record<string, any> | null> {
  ...
  // Delete a file
  async deleteFile(path: string, message: string, branch: string, sha: string): Promise<Record<string, any> | null> {
  ...
  async createCommit(message: string, changes: FileChange[], branch: string, authorName: string, authorEmail: string): Promise<Record<string, any> | null> {
  ...
  async createPullRequest(title: string, body: string, head: string, base: string | null = null): Promise<Record<string, any> | null> {
  ...
  // Get a pull request
  async getPullRequest(number: number): Promise<Record<string, any> | null> {
    return this.request('GET', `/repos/${this.owner}/${this.repo}/pulls/${number}`);
  }

  // Merge a pull request
  async mergePullRequest(number: number, commitTitle: string, mergeMethod: string = 'squash'): Promise<Record<string, any> | null> {
  ...
  async closePullRequest(number: number): Promise<Record<string, any> | null> {
  ...
  async addPullRequestComment(number: number, body: string): Promise<Record<string, any> | null> {
  ...
  async getPullRequestReviews(number: number): Promise<Record<string, any> | null> {
...
export class ForgeGitHubService {
  ...
  async getRepoForForge(forgeId: string): Promise<GitHubRepo> {
  ...
  async createPatchPR(patchId: string): Promise<Record<string, any> | null> {
...
[View on BotHub](https://bothub.dev/patches/${patchId})`;
  ...
  // Merge a patch's PR
  async mergePatchPR(patchId: string): Promise<{ success: boolean }> {
    const patchResult = await this.db.query(
      `SELECT p.*, f.github_repo, f.github_app_installation_id
       FROM patches p
       JOIN forges f ON f.id = p.forge_id
  ...
  async closePatchPR(patchId: string): Promise<{ success: boolean }> {
    const patchResult = await this.db.query(
      `SELECT p.*, f.github_repo, f.github_app_installation_id
       FROM patches p
       JOIN forges f ON f.id = p.forge_id
  ...
  async syncReviewsToGitHub(patchId: string): Promise<void> {
    const patchResult = await this.db.query(
      `SELECT p.*, f.github_repo, f.github_app_installation_id
       FROM patches p
       JOIN forges f ON f.id = p.forge_id
