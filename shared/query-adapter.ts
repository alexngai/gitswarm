 ...
 */
import type { QueryFn, QueryResult, TableResolver, TableMap } from './types.js';

/** Minimal interface for a SQLite store that query-adapter wraps. */
 ...
 *
 * @param store - Object with .query(sql, params) method
 * @returns Async query function compatible with shared services
 */
export function createSqliteAdapter(store: SqliteStore): QueryFn {
  return async function query(sql: string, params: unknown[] = []): Promise<QueryResult> {
    ...
    const sqliteSQL = sql.replace(/\$(\d+)/g, (_: string, n: string): string => {
      indices.push(parseInt(n, 10) - 1); // $1 -> index 0
      return '?';
    });
    // Reorder params to match appearance order
    const reordered: unknown[] = indices.length > 0
 ...
 *
 * @param tableMap - Map of logical name to actual table name
 * @returns t(logicalName) => actualTableName
 */
export function createTableResolver(tableMap: TableMap): TableResolver {
  return function t(name: string): string {
    const resolved = tableMap[name as keyof TableMap];
