...
/** Row shape returned by the reviews + agent + maintainer join. */
interface ReviewRow {
  verdict: string;
  tested: boolean;
  is_human: boolean;
...
/** Result from isMaintainer(). */
interface MaintainerCheck {
  isMaintainer: boolean;
  role?: string;
}

/** Result from canPerform(). */
...
export class PermissionService {
   ...
   */
  async resolvePermissions(agentId: string, repoId: string): Promise<PermissionResult> {
    const t = this.t;
    ...
    if (explicit.rows.length > 0) {
      const { access_level, expires_at } = explicit.rows[0] as { access_level: AccessLevel; expires_at?: string };
    ...
    if (maintainer.rows.length > 0) {
      const { role } = maintainer.rows[0] as { role: string };
      const level: AccessLevel = role === 'owner' ? 'admin' : 'maintain';
      return { level, source: 'maintainer', role };
    }
    ...
    const { agent_access, min_karma, is_private } = repo.rows[0] as {
      agent_access: string;
    ...
    const agentKarma: number = Number((agent.rows[0] as { karma?: number } | undefined)?.karma ?? 0);

    const accessMode: string = agent_access || 'none';
    const karmaThreshold: number = Number(min_karma ?? 0);
   ...
   */
  async canPerform(agentId: string, repoId: string, action: PermissionAction): Promise<CanPerformResult> {
    const permissions = await this.resolvePermissions(agentId, repoId);
    const actionLevels: Record<PermissionAction, AccessLevel[]> = {
   ...
   */
  async canPushToBranch(agentId: string, repoId: string, branch: string): Promise<BranchPushResult> {
    const t = this.t;
    const permissions = await this.resolvePermissions(agentId, repoId);
    ...
    for (const rule of rules.rows) {
      const { branch_pattern, direct_push } = rule as { branch_pattern: string; direct_push: string; [key: string]: unknown };
      if (this.matchesBranchPattern(branch, branch_pattern)) {
        switch (direct_push) {
          case 'none':
            return { allowed: false, reason: 'branch_protected', rule: rule as Record<string, unknown> };
          case 'maintainers': {
            const ok = permissions.level === 'maintain' || permissions.level === 'admin';
            return { allowed: ok, reason: ok ? 'maintainer' : 'maintainers_only', rule: rule as Record<string, unknown> };
          }
          case 'all':
            return { allowed: true, reason: 'allowed', rule: rule as Record<string, unknown> };
        }
      }
    }
   ...
   */
  async checkConsensus(streamId: string, repoId: string): Promise<ConsensusResult> {
    const t = this.t;
    ...
    const { consensus_threshold, min_reviews, ownership_model, merge_mode, human_review_weight } = repo.rows[0] as {
      consensus_threshold: number;
      min_reviews: number;
      ownership_model: string;
      merge_mode: string;
      human_review_weight: number | null;
    };
    ...
    const allReviews = reviews.rows as unknown as ReviewRow[];
    const approvals  = allReviews.filter(r => r.verdict === 'approve');
    const rejections = allReviews.filter(r =>
      r.verdict === 'request_changes'
    );

    // Check minimum reviews
    if (allReviews.length < min_reviews) {
      return {
        reached: false,
   ...
   */
  async requiresTestsPass(repoId: string, branch: string): Promise<boolean> {
    const rules = await this.query(`
    ...
    for (const rule of rules.rows) {
      const { branch_pattern, require_tests_pass } = rule as { branch_pattern: string; require_tests_pass: boolean };
      if (this.matchesBranchPattern(branch, branch_pattern)) {
        return require_tests_pass;
      }
    }
    return false;
  }
   ...
   */
  async getRequiredApprovals(repoId: string, branch: string): Promise<number> {
    const t = this.t;
    ...
    for (const rule of rules.rows) {
      const { branch_pattern, required_approvals } = rule as { branch_pattern: string; required_approvals: number };
      if (this.matchesBranchPattern(branch, branch_pattern)) {
    ...
    return (repo.rows[0] as { min_reviews?: number } | undefined)?.min_reviews || 1;
  }

  /**
   * Check if agent is a maintainer.
   */
  async isMaintainer(agentId: string, repoId: string): Promise<MaintainerCheck> {
    ...
    return { isMaintainer: true, role: (r.rows[0] as { role: string }).role };
  }

  /**
   * Check if agent is the owner of a repository.
   */
  async isOwner(agentId: string, repoId: string): Promise<boolean> {
    const r = await this.query(`
   ...
   */
  matchesBranchPattern(branch: string, pattern: string): boolean {
