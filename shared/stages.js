...
export class StageService {
  ...
  async updateRepoMetrics(repoId) {
    ...
    await this.query(`
      UPDATE ${t('repos')} SET
        ...
        updated_at = NOW()
      WHERE id = $3
    `, [parseInt(contributors.rows[0].count), parseInt(streams.rows[0].count), repoId]);
  ...
  async checkAllReposForAdvancement() {
    ...
    const results = { checked: repos.rows.length, advanced: 0, details: [] };

    for (const repo of repos.rows) {
      const elig = await this.checkAdvancementEligibility(repo.id);
      if (elig.eligible) {
        const adv = await this.advanceStage(repo.id);
        if (adv.success) {
          results.advanced++;
          results.details.push({ repo_id: repo.id, from: adv.previous_stage, to: adv.new_stage });
        }
      }
    }
