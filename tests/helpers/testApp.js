...
function handleSelect(store, sql, params) {
...
function handleInsert(store, sql, params) {
  const sqlLower = sql.toLowerCase();
...
function handleUpdate(store, sql, params) {
  const sqlLower = sql.toLowerCase();
...
function handleDelete(store, sql, params) {
  const sqlLower = sql.toLowerCase();
...
export function createMockRedis() {
  ...
  return {
    ...
    zrange: async (key, start, end, ...args) => {
...
async function registerAgentRoutes(app, db, redis, prefix) {
...
async function registerHiveRoutes(app, db, redis, prefix) {
  const authenticate = createAuthMiddleware(db);
  const rateLimit = createRateLimiter();
...
async function registerPostRoutes(app, db, redis, prefix) {
  const authenticate = createAuthMiddleware(db);
  const rateLimit = createRateLimiter();
...
async function registerCommentRoutes(app, db, redis, prefix) {
  const authenticate = createAuthMiddleware(db);
  const rateLimit = createRateLimiter();
...
async function registerKnowledgeRoutes(app, db, redis, prefix) {
  const authenticate = createAuthMiddleware(db);
  const rateLimit = createRateLimiter();
...
async function registerForgeRoutes(app, db, redis, prefix) {
  const authenticate = createAuthMiddleware(db);
  const rateLimit = createRateLimiter();
...
async function registerPatchRoutes(app, db, redis, prefix) {
  const authenticate = createAuthMiddleware(db);
  const rateLimit = createRateLimiter();
...
async function registerBountyRoutes(app, db, redis, prefix) {
  const authenticate = createAuthMiddleware(db);
  const rateLimit = createRateLimiter();
...
async function registerSyncRoutes(app, db, redis, prefix) {
  const authenticate = createAuthMiddleware(db);
  const rateLimit = createRateLimiter();
