...
export class PermissionService {
  ...
  async resolvePermissions(agentId: string, repoId: string): Promise<PermissionResult> {
    ...
    if (explicit.rows.length > 0) {
      const { access_level, expires_at } = explicit.rows[0] as { access_level: string; expires_at: string | null };
    ...
    if (maintainer.rows.length > 0) {
      const role = maintainer.rows[0].role as string;
      const level = role === 'owner' ? 'admin' : 'maintain';
   ...
   */
  async canPerform(agentId: string, repoId: string, action: string): Promise<CanPerformResult> {
    const permissions = await this.resolvePermissions(agentId, repoId);
    const actionLevels: Record<string, string[]> = {
  ...
  async canPushToBranch(agentId: string, repoId: string, branch: string): Promise<BranchPushResult> {
    ...
    for (const rule of rules.rows) {
      const ruleData = rule as { branch_pattern: string; direct_push: string };
      if (this.matchesBranchPattern(branch, ruleData.branch_pattern)) {
        switch (ruleData.direct_push) {
          case 'none':
  ...
  async checkConsensus(streamId: string, repoId: string): Promise<ConsensusResult> {
    ...
    for (const r of approvals) {
      approvalWeight += r.is_human
        ...
        : Math.sqrt((r.karma as number || 0) + 1);
    }
   ...
   */
  async requiresTestsPass(repoId: string, branch: string): Promise<boolean | number> {
    const rules = await this.query(`
    ...
    for (const rule of rules.rows) {
      const ruleData = rule as { branch_pattern: string; require_tests_pass: number };
      if (this.matchesBranchPattern(branch, ruleData.branch_pattern)) {
        return ruleData.require_tests_pass;
      }
    }
    return false;
  }
  ...
  async getRequiredApprovals(repoId: string, branch: string): Promise<number> {
    ...
    for (const rule of rules.rows) {
      const ruleData = rule as { branch_pattern: string; required_approvals: number };
      if (this.matchesBranchPattern(branch, ruleData.branch_pattern)) {
        return ruleData.required_approvals;
      }
    }
    ...
    return (repo.rows[0]?.min_reviews as number) || 1;
  }

  /**
   * Check if agent is a maintainer.
   */
  async isMaintainer(agentId: string, repoId: string): Promise<MaintainerResult> {
    ...
    return { isMaintainer: true, role: r.rows[0].role as string };
  }
